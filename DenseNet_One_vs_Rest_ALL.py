'''
This script runs a OvR classification for all subjects pooled together, for the desired data set, image encoding,
 and target word. Refer to DenseNet_One_vs_Rest2.py for more detail (this is a modification for all subjects, rather
 than subject dependent).

 Run_DNet_ALL.py is the driver script.
'''
...
def main(gaf, word, method):
    ...
    subjects = ['MM05', 'MM08', 'MM09', 'MM10', 'MM11', 'MM12', 'MM14', 'MM15', 'MM16', 'MM18', 'MM19', 'MM20', 'MM21',
                'P02']
    ...
    df = pd.DataFrame(columns=['prompt', 'images'])

    for subject in subjects:

        df1 = pd.read_csv(PATH_TO_DATA + subject + "\\kinect_data\\labels.txt", delimiter="\n", header=None)
        ...
        df1 = df1.loc[df1['prompt'].isin(words)]

        df1['prompt'] = pd.Series(np.where(df1['prompt'] == target, 1, 0), df1.index)

        df = df.append(df1, ignore_index = True)
        del df1

    print("df Values")
    print(df['prompt'].count())
    ...
    print("image shape is: {}".format(train_x[0].shape))
    ...
    val_x = tf.convert_to_tensor(np.asarray(val_x.tolist()))
    ...
    if method == 'DTWCT':
        ...
        batch = 4           #gpu dependent, compounded by image size
